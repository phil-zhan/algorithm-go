...
import (
	"algorithm-go/src/class04"
	"fmt"
)

func main() {
	var arr = []int{1, 2, 3, 2, 2, 2, 2, 1, 1, 1, 5, 6, 7, 3}
	class04.MergeSort(&arr)
	fmt.Println("排序后的结果:")
	for i := 0; i < len(arr); i++ {
		fmt.Print(arr[i])
	}
	...
	sum := class04.GetSmallSum(&arr2)
	fmt.Println("最小和的结果")
	fmt.Println(sum)

	fmt.Println("==========================================")
	pairs := class04.ReversePairs([]int{1, 3, 2, 5, 6, 8})
	fmt.Println("逆序对的结果")
	fmt.Println(pairs)
	...
	biggerTwice := class04.BiggerTwice(&arr3)
	fmt.Println("大于两倍的结果")
